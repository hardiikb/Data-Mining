...
def get_tree_node(data):
    ...
    node = TreeNode()
    columns = random.sample(range(0, len(data[0])-1),random_features)
    for col in columns:
...
number_of_trees = int(input("Enter the number of trees in the forest"))
random_features = int(len(df[0]) / 5)
...
for fold in range(len(cross_valid)):
    ...
    predicted_classes = []
    entire_forest_results = []
    for i in range(number_of_trees):
        sample_train = train[np.random.choice(len(train), len(train), replace=True),:]
        root = get_tree_node(data=train)
        root = generate_decision_tree(root, 1)
        test_class = []
        for test_row in test:
            test_class.append(predict(root, test_row))
        entire_forest_results.append(test_class)
    entire_forest_results = np.transpose(entire_forest_results)
    output = []
    for datapoint in range(len(entire_forest_results)):
        data = list(entire_forest_results[datapoint])
        output.append(max(set(data), key=data.count))
    metrics.get_values(output, test[:,-1])
